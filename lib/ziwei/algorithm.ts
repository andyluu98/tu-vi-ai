...
export function generateChart(birthInfo: BirthInfo): ZiweiChart {
  ...
  const palaces: Palace[] = astrolabe.palaces.map(p => {
    ...
    const allStars: Star[] = [
      ...
      // 3 vòng thần sát iztro (mỗi cung 1 sao mỗi vòng): Trường Sinh, Bác Sĩ, Tướng tiền, Tuế tiền
      ...([p.changsheng12, p.boshi12, p.jiangqian12, p.suiqian12]
        .filter(Boolean)
        .map(nm => ({ name: nm as string, type: 'minor' as const }))),
    ];

    // Khử trùng lặp theo tên: vài sao có ở cả tạp diệu lẫn vòng thần sát (Hoa Cái, Hàm Trì, Thiên Đức...)
    const seen = new Set<string>();
    const uniqueStars = allStars.filter(s => (seen.has(s.name) ? false : (seen.add(s.name), true)));
  ...
  // ── 小限（Tiểu hạn）: iztro cung cấp sẵn p.ages (mảng tuổi rơi vào cung này) ──
  // palaces[] giữ nguyên thứ tự/độ dài với astrolabe.palaces (cùng map ở trên) nên dò theo index.
  astrolabe.palaces.forEach((p, i) => {
    const ages = p.ages as number[] | undefined;
    if (ages?.includes(currentAge)) {
      palaces[i].isCurrentTieuHan = true;
    }
  });
  ...
  // ── Tuần / Triệt (Tử Vi Việt: an theo can-chi năm sinh; iztro hệ Tàu không có) ──
  const yearly = astrolabe.rawDates?.chineseDate?.yearly as string[] | undefined;
  const yStem = yearly ? STEMS.indexOf(yearly[0]) : -1;
  const yBranch = yearly ? BRANCHES.indexOf(yearly[1]) : -1;
  if (yStem >= 0 && yBranch >= 0) {
    // Triệt theo can năm (mod 5): Giáp/Kỷ→Thân Dậu, Ất/Canh→Ngọ Mùi, Bính/Tân→Thìn Tỵ, Đinh/Nhâm→Dần Mão, Mậu/Quý→Tý Sửu
    const trietBranches = [[8, 9], [6, 7], [4, 5], [2, 3], [0, 1]][yStem % 5];
    // Tuần: 2 chi không vong của tuần chứa năm sinh (旬首 = branch - stem)
    const xunHead = (yBranch - yStem + 12) % 12;
    const tuanBranches = [(xunHead + 10) % 12, (xunHead + 11) % 12];
    palaces.forEach(p => {
      p.isTriet = trietBranches.includes(p.branch);
      p.isTuan = tuanBranches.includes(p.branch);
    });
  }
