...
export default function HemingPage() {
  ...
  const runAnalysis = useCallback(async (q?: string) => {
    ...
    if (!isFormReady(formA) || !isFormReady(formB)) {
      setFormError('Vui lòng nhập đầy đủ thông tin sinh của cả hai người.');
      return;
    }
    setAnalyzing(true);
    setAnalysis('');
    ...
    try {
      ...
      let cB = chartB;
      try {
        if (!cA) cA = generateChart(formToBirthInfo(formA!));
        if (!cB) cB = generateChart(formToBirthInfo(formB!));
      } catch {
        cA = null;
        cB = null;
      }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-0)' }}>
      ...
      <header style={{
        ...
        <span style={{ fontSize: '12px', color: 'var(--ac)', letterSpacing: '0.2em' }}>Phân tích hợp lá số</span>
        <div style={{ flex: 1 }} />
        <span style={{ fontSize: '11px', color: 'var(--tx-3)' }}>Tình cảm · Hợp tác · Cha mẹ con · Bạn bè</span>
      </header>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 24px 80px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '36px' }}>
          ...
          <p style={{ fontSize: '13px', color: 'var(--tx-3)', lineHeight: 1.6 }}>
            Nhập thông tin sinh của hai người, AI theo hệ Nghê Hải Hạ phân tích mức hợp duyên, xu hướng tình cảm và lời khuyên tương xử
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '24px' }}
          ...
          {/* 甲方 */}
          <div style={cardStyle}>
            <span style={labelStyle}>Người A</span>
            <BirthForm
              hideSubmit
              onSubmit={() => {}}
              onFormSave={setFormA}
          ...
          {/* 乙方 */}
          <div style={cardStyle}>
            <span style={labelStyle}>Người B</span>
            <BirthForm
              hideSubmit
              onSubmit={() => {}}
              onFormSave={setFormB}
        ...
        {analysis && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {[
                'Mức độ hợp về tình cảm ra sao?',
                'Có hợp để hợp tác khởi nghiệp không?',
                'Hai người kết hôn có hợp không?',
                'Dễ mâu thuẫn nhất ở mặt nào?',
                'Tài vận có bổ trợ nhau không?',
              ].map(q => (
                <button
                  key={q}
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <input
                ...
                placeholder="Hỏi tiếp, ví dụ: những năm nào là mốc then chốt tình cảm của hai người?"
                disabled={analyzing}
              ...
              >
                {analyzing ? 'Đang phân tích…' : 'Hỏi tiếp'}
              </button>
            </div>
          </div>
        )}
