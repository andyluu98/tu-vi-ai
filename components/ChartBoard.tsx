...
import { hanVietBranch, hanVietPalace, hanVietJu } from '@/lib/ziwei/han-viet';
import PalaceCell from './PalaceCell';
import { type TimeView, getYearStemIndex, buildSiHuaOverlay } from './TimeNav';

interface ChartBoardProps {
  chart: ZiweiChart;
  view: TimeView;          // chế độ thời gian (do page.tsx quản lý, 1 nguồn duy nhất)
...
export default function ChartBoard({ chart, view, liunianYear, onStarSelect, onPalaceSelect, onSiHuaClick }: ChartBoardProps) {
  ...
  return (
    <div className="w-full select-none">
      {/* TimeNav đã chuyển lên page.tsx (1 nguồn state duy nhất); ChartBoard chỉ nhận view/liunianYear qua props */}
      ...
      >
        ...
        <h2 className="text-sm tracking-[0.25em] font-medium" style={{ color: 'var(--t-gold)' }}>
          {chart.birthInfo.name ? `${chart.birthInfo.name} · ` : ''}Lá số Tử Vi Đẩu Số
        </h2>
      </motion.div>
        ...
        >
          ...
          <div className="text-center space-y-1">
            ...
            <div className="text-[10px] space-y-0.5" style={{ color: 'var(--t-faint)' }}>
              <div>Mệnh <span style={{ color: 'var(--t-gold)', opacity: 0.7 }}>{hanVietBranch(chart.mingGongBranch)}</span></div>
              <div>Thân <span className="text-sky-500/70">{hanVietBranch(chart.shenGongBranch)}</span></div>
              <div className="text-[9px]" style={{ color: 'var(--t-gold)', opacity: 0.75 }}>{hanVietJu(chart.wuxingJuName)}</div>
            </div>
          </div>
          ...
          {chart.currentDaXianIndex >= 0 && (() => {
            ...
            return (
              <div className="border border-purple-500/30 rounded-lg px-3 py-1.5 text-center"
                ...
                <div className="text-[9px] text-purple-500/60">{hanVietPalace(dx.palaceName)}</div>
              </div>
            );
          })()}

          <div className="text-[8px] text-center leading-relaxed font-mono" style={{ color: 'var(--t-faint)', opacity: 0.75 }}>
            ÂL {chart.lunarInfo.lunarYear}·{chart.lunarInfo.isLeapMonth ? 'nhuận ' : ''}
      ...
      >
        ...
        <span className="px-1.5 py-0.5 rounded-full" style={{ color: 'var(--t-faint)', border: '1px solid var(--t-border)' }}>
          Bấm 1 cung để AI luận cung đó + xem tam phương tứ chính
        </span>
      </motion.div>
    </div>
  );
