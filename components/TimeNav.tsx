...
import { hanVietStar, hanVietSihua, hanVietStemName } from '@/lib/ziwei/han-viet';
      ...
      {overlayInfo && (
        ...
        >
          <span className="text-[9px]" style={{ color: 'var(--t-faint)' }}>
            {view === 'daxian' ? 'Đại hạn' : `${liunianYear}`}·{hanVietStemName(overlayInfo.stemName)} tứ hóa:
          </span>
          {(['禄', '权', '科', '忌'] as const).map(sh => {
            ...
            return (
              <span key={sh} className="text-[9px] font-medium" style={{ color: SIHUA_COLORS[sh] }}>
                {hanVietStar(starName)} hóa {hanVietSihua(sh)}
              </span>
            );
          })}
        </motion.div>
