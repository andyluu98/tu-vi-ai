...
import { hanVietBranch } from '@/lib/ziwei/han-viet';
...
function calcTrueSolarBranch(clockHour: number, clockMinute: number, longitude: number): number {
  ...
  const offset = (longitude - 105) * 4; // 105°E = kinh tuyến chuẩn giờ VN (UTC+7)
...
export default function BirthForm({ onSubmit, loading, initialData, onFormSave, hideSubmit }: BirthFormProps) {
  ...
  const [form, setForm] = useState<BirthFormState>({
    ...
    longitude: initialData?.longitude ?? 105.85,
  ...
  const offsetMin = Math.round((form.longitude - 105) * 4);
  ...
  const errors = {
    year: !form.year ? 'Chọn năm sinh'
      : y < 1900 || y > 2026 ? 'Năm hợp lệ: 1900–2026'
      : '',
    month: !form.month ? 'Chọn tháng' : '',
    day: !form.day ? 'Chọn ngày'
  ...
  const summaryText = showSummary
    ? [
        ...
        form.unknownTime ? 'Không rõ giờ' : `giờ ${hanVietBranch(branch)}`,
  ...
  const handleProvince = (prov: string) => {
    ...
    setForm({ ...form, province: prov, city: firstCity?.name || '', longitude: firstCity?.longitude ?? 105 });
  };

  const handleCity = (cityName: string) => {
    const prov = PROVINCES.find(p => p.name === form.province);
    const cityData = prov?.cities.find(c => c.name === cityName);
    setForm({ ...form, city: cityName, longitude: cityData?.longitude ?? 105 });
  };
  ...
  const inputStyle = {
    ...
    colorScheme: isDark ? 'dark' : 'light', // dropdown native đúng nền tối/sáng
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: '16px' }}>
        <label style={{ display: 'block', fontSize: '11px', color: labelClr, marginBottom: '6px', letterSpacing: '0.05em' }}>Họ tên (không bắt buộc)</label>
        <input
          type="text"
      ...
      {/* ── 出生地点 ── */}
      <div style={{ marginBottom: '16px' }}>
        <label style={{ display: 'block', fontSize: '11px', color: labelClr, marginBottom: '6px', letterSpacing: '0.05em' }}>Nơi sinh (để hiệu chỉnh giờ mặt trời thật)</label>
        ...
        <AnimatePresence mode="wait">
          {form.province ? (
            ...
            >
              {form.city || '(chọn thành phố)'} · Kinh độ {form.longitude.toFixed(2)}°E · Lệch {offsetMin > 0 ? '+' : ''}{offsetMin} phút
            </motion.p>
          ) : (
            <motion.p
              key="location-hint"
            ...
            >
              * Nghê Hải Hạ luận theo giờ mặt trời thật; nên nhập nơi sinh để tự hiệu chỉnh giờ
            </motion.p>
          )}
        </AnimatePresence>
      </div>
      ...
      {/* ── 出生时间 ── */}
      <div style={{ marginBottom: '16px' }}>
        <label style={{ display: 'block', fontSize: '11px', color: labelClr, marginBottom: '6px', letterSpacing: '0.05em' }}>Giờ sinh (theo đồng hồ)</label>
        <div style={{ borderRadius: '14px', padding: '12px', background: panelBg, border: `1px solid ${panelBorder}`, opacity: form.unknownTime ? 0.45 : 1, pointerEvents: form.unknownTime ? 'none' : 'auto', transition: 'opacity 0.2s' }}>
          ...
          <div style={{ textAlign: 'center', padding: '4px 0' }}>
            <span style={{ fontSize: '10px', color: isDark ? 'rgba(170,195,220,0.75)' : 'rgba(140,100,20,0.5)' }}>Giờ mặt trời thật → </span>
            <span style={{ fontSize: '15px', color: goldText, fontWeight: 600, letterSpacing: '0.08em' }}>
              giờ {hanVietBranch(branch)}
            </span>
            {shichenInfo && (
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: '7px', marginTop: '8px', cursor: 'pointer' }}>
          ...
          <span style={{ fontSize: '10px', color: isDark ? 'rgba(165,185,210,0.7)' : 'rgba(140,100,20,0.45)' }}>
            Không biết giờ sinh, lập lá số theo giờ Tý (23:00–01:00)
          </span>
        </label>
      </div>
