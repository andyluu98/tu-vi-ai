...
import { hanVietStar, hanVietPalace, hanVietStem, hanVietBranch, hanVietSihua } from '@/lib/ziwei/han-viet';
...
export default function PalaceCell({
  ...
  const { branch, stem, name, stars, daXianAge, isCurrentDaXian, isMingGong, isShenGong, isTuan, isTriet, isCurrentTieuHan } = palace;
  const ganzhi = `${hanVietStem(stem)} ${hanVietBranch(branch)}`;
  ...
  const minorStars = stars.filter(s => s.type === 'minor'); // tạp diệu + 3 vòng thần sát
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1 mb-0.5 pr-8">
        ...
        {isMingGong && (
          <span className="text-[7px] text-amber-500/80 border border-amber-500/30 px-0.5 rounded leading-tight">Mệnh</span>
        )}
        {isShenGong && (
          <span className="text-[7px] text-sky-500/80 border border-sky-500/30 px-0.5 rounded leading-tight">Thân</span>
        )}
        {isTriet && (
          <span className="text-[7px] font-bold text-red-600 border border-red-500/50 bg-red-500/10 px-0.5 rounded leading-tight">Triệt</span>
        )}
        {isTuan && (
          <span className="text-[7px] font-bold text-violet-600 border border-violet-500/50 bg-violet-500/10 px-0.5 rounded leading-tight">Tuần</span>
        )}
        {isCurrentTieuHan && (
          <span className="text-[7px] font-bold text-green-600 border border-green-500/50 bg-green-500/10 px-0.5 rounded leading-tight">Tiểu hạn</span>
        )}
      </div>
      ...
      <div className="flex flex-col gap-0.5 flex-1">
        {majorStars.length === 0 && (
          <span className="text-[10px] italic" style={{ color: 'var(--t-faint)', opacity: 0.6 }}>Vô chính diệu</span>
        )}
        {majorStars.map((star) => {
        ...
        <div className="flex flex-wrap gap-x-1">
          {shaStars.map(s => (
            <span key={s.name} className="text-[9px] text-red-500/60 leading-tight">
              {hanVietStar(s.name)}{s.siHua && <SiHuaBadge siHua={s.siHua} />}
            </span>
          ))}
        </div>
      )}

      {/* Tạp diệu + thần sát (Trường Sinh / Bác Sĩ / lưu niên) */}
      {minorStars.length > 0 && (
        <div className="flex flex-wrap gap-x-1 gap-y-0 mt-0.5">
          {minorStars.map((s, i) => (
            <span key={`${s.name}-${i}`} className="text-[8px] leading-tight" style={{ color: 'var(--t-faint)', opacity: 0.7 }}>
              {hanVietStar(s.name)}
            </span>
          ))}
        </div>
